const About = () => {
  return (
    <div className="page about">
      <div className="container">
        <h1>About BlogIt</h1>

        <p>
          Welcome to <strong>BlogIt</strong>, your personal microblogging space!
        </p>

        <p>
          <strong>BlogIt</strong> is a simple and easy-to-use platform where you
          can write and share your thoughts, ideas, or everyday moments.
          It&apos;s your own personal corner of the internet to share quick
          updates, stay connected with family and friends, or just enjoy writing
          for yourself
        </p>

        <p>
          No distractions, no extra noise—just a cozy place for your own blogs!
        </p>

        <h2>What is Microblogging?</h2>
        <p>
          Microblogging is a way to share short posts about whatever&apos;s on
          your mind. You don&apos;t need to write long posts—just a few words, a
          picture, or a link, and you&apos;re good to go! It&apos;s a fast and
          easy way to stay connected and share what&apos;s on your mind without
          too much effort. It&apos;s perfect for when you want to express
          something in the moment, whether it&apos;s a random thought, a quick
          update, or something cool you found online.
        </p>

        <p>
          Ready to start blogging? Blog with <strong>BlogIt</strong> today,
          because <em>&quot;Each post is a canvas for your voice!&quot;</em>
        </p>

        <p>Thanks for visiting, and happy blogging!</p>
      </div>
    </div>
  );
};

export default About;
